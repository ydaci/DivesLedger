...
contract DivesLedger is ERC721, Ownable {
    ...
    constructor() ERC721("DivesLedger", "DLG") Ownable(msg.sender) {}
    ...
    function getCertificationName(
        ...
        if (_certLevel == 2)
            return unicode"2. Diver 2 stars / Advanced Open Water Diver";
    ...
    function addCertification(
        ...
        for (uint i = 0; i < certifications.length; i++) {
            if (certifications[i].certLevel == _certLevel) {
                revert(
                    "Certification with the same level already exists for this diver."
                );
            }
        }

        diverCertifications[msg.sender].push(
            Certification({
        ...
        emit CertificationAdded(
            msg.sender,
            getCertificationName(_certLevel),
    ...
    function getCertifications(
        address _diver
    ) public view returns (Certification[] memory) {
    ...
    function getDivesByDiver(
        address _diver
    ) public view returns (Dive[] memory) {
